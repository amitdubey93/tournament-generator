...
//    @Query("select playerId, sum(score) as score, sum(won) as won, sum(matchPlayed) as matchPlayed, round(( sum(won)/sum(matchPlayed) * 100 ),2) as winPercent, sum(lost) as lost, tournament.tournamentId from PointsTable group by playerId")
    @Query("select new io.h2o.ufc.model.Player(pl.playerId, " +
            "pl.playerName, " +
            "cast(ifnull(sum(pt.matchPlayed),0) as integer), " +
            "cast(ifnull(sum(pt.score), 0) as integer), " +
            "cast(ifnull(sum(pt.won), 0) as integer) " +
//            "cast(ifnull(round(( sum(pt.won)/sum(pt.matchPlayed) * 100 ),4), 0) as float ),  " +
//            "cast(ifnull(cast(ifnull(sum(pt.score), 0) as integer)/cast(ifnull(sum(pt.matchPlayed), 1) as integer), 2) as float)) " +
